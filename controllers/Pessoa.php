...
namespace controllers {
    ...
    class Pessoa
    {
        ...
        {
            global $app;
            $sth = $this->PDO->prepare("SELECT * FROM produtos WHERE id = :id");
            $sth->bindValue(':id', $id);
            $sth->execute();
        ...
        public function nova()
        {
             ...
             */
            $sth = $this->PDO->prepare("INSERT INTO produtos (" . implode(',', $keys) . ") VALUES (:" . implode(",:", $keys) . ")");
        ...
        public function editar($id)
        {
            ...
            $sth = $this->PDO->prepare("UPDATE produtos SET " . implode(',', $sets) . " WHERE id = :id");
            $sth->bindValue(':id', $id);
            foreach ($dados as $key => $value) {
                $sth->bindValue(':' . $key, $value);
            }
        ...
        {
            global $app;
            $sth = $this->PDO->prepare("DELETE FROM produtos WHERE id = :id");
            $sth->bindValue(':id', $id);
            $app->render('default.php', ["data" => ['status' => $sth->execute() == 1]], 200);
        }
    }
